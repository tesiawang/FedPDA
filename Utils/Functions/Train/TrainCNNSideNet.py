...
class TrainCNNSideNet:
    def __init__(self,
        ...
        self._epoch_loss_list = np.zeros(max_epoches, dtype=np.float32)
    ...
    def train_one_batch(self,
        ...
        grads = tape.gradient(batch_loss, self._side_net.trainable_variables)
        self._optimizer.apply_gradients(zip(grads, self._side_net.trainable_variables))
        return batch_loss
    ...
    def train(self, stopping_num: int=5, save_mode:str='epoch'):
        ...
        for epoch in range(self._max_epoches):
            ...
            for batch_data_path in self._train_data_pathes:
                ...
                if save_mode=='iteration':
                    save_pkl_file(self._side_net.get_weights(), Path(self._model_save_folder).joinpath("updated_parameters_%d.pkl" % checkpoint_idx))
                    checkpoint_idx += 1

                epoch_loss += batch_loss
            self._epoch_loss_list[epoch] = epoch_loss/(len(self._train_data_pathes)*batch_N0.shape[0])
            log_print("|====> CNN Side Net Training, Epoch %d, Loss: %.4f" % (epoch, self._epoch_loss_list[epoch]), color='g')
        ...
        return self._side_net.get_weights()