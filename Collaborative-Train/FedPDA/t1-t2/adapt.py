...
parser.add_argument("--cache_folder", type=str, default=root_path+"/Collaborative-Train/FedPDA/t1-t2/Cache-0.80-0.80", required=False)
parser.add_argument("--GCKD_folder", type=str, default=root_path+"/Collaborative-Train/FedPDA/t0-t1/Pruned-Data/0.80", required=False)
parser.add_argument("--BS_agg_ratio", type=float, default=0.8, required=False)
...
if __name__=='__main__':
    ...
    os.mkdir(Path(run_args.cache_folder).joinpath("%d" % id_BS))
    initial_global_model_parameters = cnn_side_net.get_weights()
    save_pkl_file(initial_global_model_parameters, Path(run_args.cache_folder).joinpath("%d" % id_BS, "global_model_parameters_0.pkl"))
    ...
    for r in range(1, run_args.max_rounds+1):
        ...
        for user_id in critical_users:
            ...
            # Load training and validation data of the current user
        ...
        # ----------------------------- Updating for GCKD ---------------------------- #
        user_aggregation_weight[id_BS] = run_args.BS_agg_ratio * len(critical_users) / (1-run_args.BS_agg_ratio)
        user_layermask[id_BS] = [1 for _ in range(len(initial_global_model_parameters))]

        # Load the global model parameters of last round
        last_round_global_model_parameters = load_pkl_file(Path(run_args.cache_folder).joinpath("%d" % id_BS, "global_model_parameters_%d.pkl" % (r-1)))
        cnn_side_net.set_weights(last_round_global_model_parameters)

        # Load training and validation data of GCKD
        train_data_pathes = get_file_pathes_in_folder(Path(run_args.GCKD_folder))

        # Training
        os.mkdir(Path(run_args.cache_folder).joinpath("%d" % id_BS, "Updated-Parameters-Round%d" % r))
        obj = TrainCNNSideNet(config=BasicConfig(),
                              main_net_infer=cnn_main_net,
                              side_net=cnn_side_net,
                              max_epoches=run_args.num_epoches,
                              ...
                              train_data_pathes=train_data_pathes,
                              val_data_pathes=None,
                              model_save_folder=Path(run_args.cache_folder).joinpath("%d" % id_BS, "Updated-Parameters-Round%d" % r))
        updated_parameters = obj.train(save_mode='epoch')
        ...
        for layer_idx in range(len(updated_parameters)):
            model_updates.append((updated_parameters[layer_idx]-last_round_global_model_parameters[layer_idx]).astype(np.float32))
        user_model_updates[id_BS] = model_updates
        ...
        save_pkl_file(updated_global_model_parameters, Path(run_args.cache_folder).joinpath("%d" % id_BS, "global_model_parameters_%d.pkl" % r))