...
parser.add_argument("--cache_folder", type=str, default=root_path+"/Collaborative-Train/Vanilla-Federated/t1-t2/NF/Cache", required=False)
...
parser.add_argument("--training_data_folder", type=str, default=root_path+"/Collaborative-Train/Online-Training-Data", required=False)
parser.add_argument("--val_data_folder", type=str, default=root_path+"/Collaborative-Train/Online-Val-Data", required=False)
...
if __name__=='__main__':
    ...
    non_critical_users = [i for i in range(12)]
    # ---------------------------------------------------------------------------- #
    ...
    for user_id in critical_users + non_critical_users:
    ...
    for r in range(1, run_args.max_rounds+1):
        ...
        for user_id in critical_users:
            ...
            eval_data_pathes = get_file_pathes_in_folder(Path(run_args.val_data_folder).joinpath('Env-2', "User-%d" % user_id))
        ...
        for user_id in non_critical_users:
            cnn_side_net.set_weights(load_pkl_file(Path(run_args.cache_folder).joinpath("%d" % user_id, "global_model_parameters_%d.pkl" % (r-1))))
            eval_data_pathes = get_file_pathes_in_folder(Path(run_args.val_data_folder).joinpath('Env-1', "User-%d" % user_id))
            obj = EvalCNNSideNet(config=BasicConfig(),
                                 main_net_infer=cnn_main_net,
                                 side_net=cnn_side_net )
            ber_rounds[r-1] += obj.eval(eval_data_pathes=eval_data_pathes)

        ber_rounds[r-1] = ber_rounds[r-1] / len(critical_users + non_critical_users)
        ...
        for user_id in critical_users + non_critical_users:
            ...
            if user_id in critical_users:
                train_data_pathes = get_file_pathes_in_folder(Path(run_args.training_data_folder).joinpath("Env-2", "User-%d" % user_id))
            else:
                train_data_pathes = get_file_pathes_in_folder(Path(run_args.training_data_folder).joinpath("Env-1", "User-%d" % user_id))
